...
public class DynamicDataWindow extends JFrame {
	...
	//向坐标系添加数据并刷新曲线
	public void addData(long t, double v, int cost, int penalty, String order) {
		setyMaxMin(v);
		time[index] = t;
		val[index++] = v;
		dataTextArea.append(fmt.format(new Date(t)) + "    fitness = " + v + "    changeSum = " + cost
				+ "    penalty = " + penalty + "    order = " + order + "\n");
	...
	public void paint(Graphics g) {
		...
		for (int xt = x0 + tickX; xt < xn; xt += tickX) {
			...
			g.drawString(Integer.toString(timescale * time), xt - (time < 10 ? 3 : 7), y0 + 20);//绘制每个标度对应数值ֵ
			if (xt >= xn - tickX) {
		...
		for (double vt = 0; vt <= eqdiviY; vt += tickY) {
			...
			g.drawLine(x0, v, x0 + xn, v);//绘制标度
			g.drawString(Float.toString((float) (yMin + (vt / eqdiviY) * (yMax - yMin))), x0 - 38, v + 5);//绘制每个标度对应数值ֵ
			if (vt >= 120) {
				g.drawString("适应度值", x0 - 50, v - 15);//绘制轴名
			}
		}

		// 绘制数据曲线
		g.setColor(Color.RED);
		Graphics2D g1 = (Graphics2D)g;  //g是Graphics对象
		g1.setStroke(new BasicStroke(2.0f));//设置线条粗细
		int xp = -1;//前一个坐标值的x;
        int vp = -1;//前一个坐标值的y;
