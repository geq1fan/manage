package com.ge.modules.sys.shiro;
...
import com.ge.modules.sys.mapper.UserMapper;
...
public class AuthenticationRealm extends AuthorizingRealm {
    ...
    /**
     * 认证  校验用户身份是否合法<br>
     * 当执行认证的时候，会自动执行该函数，将AuthenticationToken参数传入<br>
     * 通过操作authenticationToken来获取账号密码，然后和数据库进行比对
     */
    @Override
...
//        );
     ...
     * 在使用@requiresPermissions或@requiresRoles注解时，会调用上述方法，从而回调该函数
     * 1、如果用户正常退出，缓存自动清空；
     * 2、如果用户非正常退出，缓存自动清空；
